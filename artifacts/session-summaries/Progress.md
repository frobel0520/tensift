...
- Top-of-page Google AdSense integration scaffold: responsive slot, public-ID validation, dev placeholder, and CSP allowlist; ads remain disabled until the publisher account and slot IDs are configured.
...
- Ad direction: reserve a top-of-page banner slot, visually separated from the game board. The integration is ready, but production still needs an approved AdSense account, `VITE_ADSENSE_CLIENT_ID`, `VITE_ADSENSE_TOP_SLOT`, `ads.txt`, and privacy/consent review.
...
- Ads are scaffolded but not monetizing until account approval and publisher configuration; analytics, privacy/consent, and release-content operations are not yet implemented.
